...
- 장소 카드(시간·카테고리 아이콘·이름·네이버지도 아이콘이 한 줄 정렬), 이동시간 카드(왼쪽이 들여쓰기된 좁은 카드)
- **후보 투표 카드**: "＋ 후보 투표"로 만들면, 한 슬롯에 여러 후보 장소를 넣고 각각에 하트를 눌러 투표할 수 있습니다. 투표수가 실시간으로 모두에게 보입니다.
- 일정 사이사이에 있는 "＋ 장소 / ＋ 이동시간 / ＋ 투표" 버튼으로 원하는 위치에 끼워넣기
- 카드를 길게 누르면(약 0.5초) 삭제 확인 팝업 — 실수 삭제 방지
- day별 "네이버지도 모음" 링크 썸네일 (본인이 만든 네이버지도 컬렉션 링크를 붙여넣으면 카드로 표시)
